...
export const getLectureAttendance = async (req: Request, res: Response) => {
    try {
        const records = await AttendanceService.getAttendanceByLecture(req.params.lectureId as string);
...
export const getStudentAttendance = async (req: Request, res: Response) => {
    try {
        const records = await AttendanceService.getAttendanceByStudent(req.params.studentId as string);
        res.status(200).json({ success: true, data: records });
    } catch (error: any) {
        res.status(400).json({ success: false, message: error.message });
    }
