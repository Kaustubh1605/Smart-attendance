...
export const getLecture = async (req: Request, res: Response) => {
    try {
        const lecture = await LectureService.getById(req.params.id as string);
        if (!lecture) {
...
export const updateLectureStatus = async (req: Request, res: Response) => {
    try {
        ...
        const lecture = await LectureService.updateStatus(req.params.id as string, req.body.status);
        res.status(200).json({ success: true, data: lecture });
    } catch (error: any) {
        res.status(400).json({ success: false, message: error.message });
    }
